...
function main(variables) {
    ...
    var downscale = 1;
    var _WIDTH         = canvas.clientWidth >>downscale;
    var _HEIGHT         = canvas.clientHeight>>downscale;
    var _WIDTH2        = _WIDTH*2|0;
    var _HEIGHT2        = _HEIGHT*2|0;
    var _NGRID         = _WIDTH*_HEIGHT|0;
    var _middle    = _WIDTH/2|0;
    var _dY        = _HEIGHT*2*_WIDTH;
    var _M         = 4*_WIDTH*_HEIGHT;

    // constants for drawing phase-plane subplots
    var plot_padding = 72>>downscale;
    var _mg           = plot_padding/2|0; // plot margin
    var _SW           = _WIDTH-plot_padding|0;   // plot width
    ...
    for (var _i_=0;_i_<fields.length   ;_i_++) buffers.push(['__'+fields[_i_].name]);
    for (var _i_=0;_i_<operators.length;_i_++) buffers.push(['__'+operators[_i_].name]);
    create_buffers = '';
    ...
    for (var _i_=0;_i_<fields.length;_i_++) {
        ...
        kernel_source += '__'+_x_+'[_i_]=min(1,max(0,'+_x_+'+__d__'+_x_+'*dt));\n';
    }
    // footer
    kernel_source += '}}}';
    ...
    kernel_source = kernel_source.replace(new RegExp('RANDN', 'g'),'(((rng.uniform()-0.5)*3.4641)/sqrt(dt))');
    ...
    function zero_finder(due,dui,_x_,_y_,size) {
        var template = `
    ...
    // derivatives
    function trace_nullclines(_dx_,_dy_,origin) {
            ...
            for (var _x_=1; _x_<_SW; _x_++) {
                ...
                if (xnull)
                    data[_x_+(_SW-_y_)*_WIDTH2+origin]=0xff00ff00;
                else if (ynull)
                    data[_x_+(_SW-_y_)*_WIDTH2+origin]=0xff0088ff;
            }
        }
    };
    ...
    function render_scatter(field__x_,field__y_,_dx_,_dy_,origin,pskip) {
        // erase this subplot WxH starting at origin
        for (var _y_=0;_y_<_WIDTH;_y_++) {
            var _Y_ = _y_*_WIDTH2+origin;
        ...
        for (var _y_=1; _y_<_HEIGHT; _y_+=pskip) {
            for (var _x_=1; _x_<_WIDTH; _x_+=pskip) {
                ...
                var a = field__x_[_i_]*_SW|0;
                var b = (1.-field__y_[_i_])*_SW|0;
                var j = a+b*_WIDTH2+origin;
                data[j] = 0x01010101*(255-((255-((data[j])&0xff))*0.5|0));
            }
        }
        trace_nullclines(_dx_,_dy_,origin);
    }
    ...
    var subplot3_template = `  
        render_scatter(__${panel3.x},__${panel3.y},pp3dx,pp3dy,${_mg+_WIDTH2*_mg+_WIDTH2*_HEIGHT});`;
    var subplot4_template = `
        render_scatter(__${panel4.x},__${panel4.y},pp4dx,pp4dy,${_mg+_WIDTH2*_mg+_WIDTH2*_HEIGHT+_WIDTH});`;
    ...
    function randomize() {
        for (var j=0;j<fields.length;j++) {
            ...
            for (var _y_=0;_y_<_HEIGHT;++_y_) {
                rng.seed();
                for (var _x_=0;_x_<_WIDTH;++_x_)
                   __${fields[j].name}[_x_+_WIDTH*_y_] = rng.uniform();
            }`);
        }
        showSim();
    }
...
function waitForMathJax() {
    ...
    for (var _i_=0;_i_<operators.length;_i_++) {
        ...
        switch (t) {
            case 'GCONV':
                eqns+=`$${n}(_x_,_y_,t)=\\frac{1}{2\\pi \\cdot ${p.sigma}}\\exp\\left[{\\frac{_x_^2+_y_^2}{${p.sigma}^2}}\\right] \\otimes ${a}$<br/>`;
                break;
            case 'LAPL':
                eqns+=`$${n}(_x_,_y_,t)=\\nabla^2 ${a}$<br/>`;
        }
